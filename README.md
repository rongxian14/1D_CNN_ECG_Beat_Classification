...
5. **GAN** – Generative Adversarial Network trained to generate synthetic ECG beats for a partially balanced dataset.
6. **SMOTE + RU** – Combined strategy to over- and under-sample for a fully balanced dataset.
7. **SMOTE + GAN** – Uses SMOTE first, followed by realistic synthetic data from GAN for a partially balanced dataset.
8. **Bootstrap + GAN** – Uses bootstrapped sampling before applying GAN-based augmentation for a partially balanced dataset.
